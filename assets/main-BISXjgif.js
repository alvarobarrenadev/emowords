(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))t(n);new MutationObserver(n=>{for(const i of n)if(i.type==="childList")for(const r of i.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&t(r)}).observe(document,{childList:!0,subtree:!0});function a(n){const i={};return n.integrity&&(i.integrity=n.integrity),n.referrerPolicy&&(i.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?i.credentials="include":n.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function t(n){if(n.ep)return;n.ep=!0;const i=a(n);fetch(n.href,i)}})();const Y="emowords_vocab",Te="emowords_settings";function j(){const e=localStorage.getItem(Y);return e?JSON.parse(e):[]}function We(e){const o=j(),a={...e,createdAt:Date.now(),lastReviewedAt:null,reviewCount:0,correctCount:0,incorrectCount:0,nextReviewAt:Date.now(),difficulty:0};o.push(a),localStorage.setItem(Y,JSON.stringify(o))}function le(e){const o=j().map(a=>a.id===e.id?e:a);localStorage.setItem(Y,JSON.stringify(o))}function Oe(e){const o=j().filter(a=>a.id!==e);localStorage.setItem(Y,JSON.stringify(o))}function He(e){return j().find(o=>o.id===e)}function Ge(e){const o=j(),a=e.toLowerCase().trim();return a?o.filter(t=>t.word.toLowerCase().includes(a)||t.meaning.toLowerCase().includes(a)||t.example&&t.example.toLowerCase().includes(a)||t.emotion&&t.emotion.toLowerCase().includes(a)||t.category&&t.category.toLowerCase().includes(a)):o}function Ee(){const e=j(),o=new Set;return e.forEach(a=>{a.category&&o.add(a.category)}),Array.from(o).sort()}function _e(e,o="date-desc"){const a=[...e];switch(o){case"date-asc":return a.sort((t,n)=>(t.createdAt||t.id)-(n.createdAt||n.id));case"date-desc":return a.sort((t,n)=>(n.createdAt||n.id)-(t.createdAt||t.id));case"alpha-asc":return a.sort((t,n)=>t.word.localeCompare(n.word));case"alpha-desc":return a.sort((t,n)=>n.word.localeCompare(t.word));case"review-count":return a.sort((t,n)=>(n.reviewCount||0)-(t.reviewCount||0));case"difficulty":return a.sort((t,n)=>(n.difficulty||0)-(t.difficulty||0));default:return a}}function ce(){const e=j(),o=e.length,a=e.filter(m=>m.remembered).length,t=o-a,n=e.reduce((m,b)=>m+(b.reviewCount||0),0),i=o>0?(n/o).toFixed(1):0,r={word:e.filter(m=>m.type==="word").length,phrasal:e.filter(m=>m.type==="phrasal").length,expression:e.filter(m=>m.type==="expression").length,connector:e.filter(m=>m.type==="connector").length},d=e.reduce((m,b)=>m+(b.correctCount||0),0),c=e.reduce((m,b)=>m+(b.incorrectCount||0),0),g=d+c>0?(d/(d+c)*100).toFixed(1):0,l=Date.now(),y=e.filter(m=>!m.nextReviewAt||m.nextReviewAt<=l).length;return{total:o,remembered:a,forgotten:t,totalReviews:n,averageReviews:i,byType:r,retentionRate:g,dueForReview:y}}const ge=[1,3,7,14,30,60,120,240];function ke(e){const o=e.correctCount||0;return o>=10?"master":o>=5?"guru":o>=2?"apprentice":"new"}function Fe(e){const o=ke(e);return{level:o,...{new:{label:"Nuevo",class:"mastery-new",icon:"fa-seedling",percent:10},apprentice:{label:"Aprendiz",class:"mastery-apprentice",icon:"fa-leaf",percent:40},guru:{label:"Experto",class:"mastery-guru",icon:"fa-tree",percent:75},master:{label:"Maestro",class:"mastery-master",icon:"fa-crown",percent:100}}[o]}}function Ue(e){if(!e.nextReviewAt)return!0;const o=new Date().setHours(23,59,59,999);return e.nextReviewAt<=o}function Ye(){const e=j(),o=Date.now();return e.filter(a=>!a.nextReviewAt||a.nextReviewAt<=o).length}function ye(e,o){const a=Date.now(),t=1440*60*1e3;if(!o)return a+600*1e3;const n=e.correctCount||0,i=Math.min(n,ge.length-1),r=ge[i],c=1-(e.difficulty||0)*.1,g=Math.max(1,Math.round(r*c));return a+g*t}function Ke(e){if(!e.nextReviewAt)return"Ahora";const o=Date.now(),a=e.nextReviewAt-o;if(a<=0)return"Ahora";const t=Math.floor(a/(60*1e3)),n=Math.floor(a/(3600*1e3)),i=Math.floor(a/(1440*60*1e3));return t<60?`${t}min`:n<24?`${n}h`:i===1?"Mañana":i<7?`${i} días`:i<30?`${Math.floor(i/7)} sem`:`${Math.floor(i/30)} mes${Math.floor(i/30)>1?"es":""}`}function Xe(){const e=j(),o=Date.now();return e.filter(t=>!t.nextReviewAt||t.nextReviewAt<=o).sort((t,n)=>{const i=o-(t.nextReviewAt||0),r=o-(n.nextReviewAt||0);if(i!==r)return r-i;const d=t.difficulty||0,c=n.difficulty||0;return d!==c?c-d:(t.reviewCount||0)-(n.reviewCount||0)})}function Je(e,o){const a=He(e);if(a)return a.remembered=o,a.lastReviewedAt=Date.now(),a.reviewCount=(a.reviewCount||0)+1,o?(a.correctCount=(a.correctCount||0)+1,a.difficulty=Math.max(-3,(a.difficulty||0)-1),a.nextReviewAt=ye(a,!0)):(a.incorrectCount=(a.incorrectCount||0)+1,a.difficulty=Math.min(3,(a.difficulty||0)+1),a.correctCount=Math.max(0,(a.correctCount||0)-2),a.nextReviewAt=ye(a,!1)),le(a),a}function Qe(){const e=j(),o={version:"1.0",exportedAt:new Date().toISOString(),wordCount:e.length,words:e};return JSON.stringify(o,null,2)}function oe(e){try{const o=JSON.parse(e);if(!o.words||!Array.isArray(o.words))throw new Error("Invalid data format: missing words array");const a=j(),t=new Set(a.map(r=>r.id));let n=0,i=0;return o.words.forEach(r=>{if(!r.word||!r.meaning){i++;return}(!r.id||t.has(r.id))&&(r.id=Date.now()+Math.random()),r.type=r.type||"word",r.remembered=r.remembered||!1,r.createdAt=r.createdAt||Date.now(),a.push(r),t.add(r.id),n++}),localStorage.setItem(Y,JSON.stringify(a)),{success:!0,imported:n,skipped:i}}catch(o){return{success:!1,error:o.message}}}function Ze(e){try{const o=e.trim().split(`
`);if(o.length<2)throw new Error("El archivo CSV debe tener al menos una fila de encabezados y una de datos");const a=ve(o[0]).map(s=>s.toLowerCase().trim()),t=a.findIndex(s=>s==="word"||s==="palabra"||s==="english"),n=a.findIndex(s=>s==="meaning"||s==="significado"||s==="spanish"||s==="traduccion"||s==="traducción");if(t===-1||n===-1)throw new Error('El CSV debe tener columnas "word" y "meaning" (o "palabra" y "significado")');const i=a.findIndex(s=>s==="type"||s==="tipo"),r=a.findIndex(s=>s==="category"||s==="categoria"||s==="categoría"),d=a.findIndex(s=>s==="example"||s==="ejemplo"),c=a.findIndex(s=>s==="emotion"||s==="emocion"||s==="emoción"||s==="association"||s==="asociacion"),g=j(),l=new Set(g.map(s=>s.word.toLowerCase().trim()));let y=0,m=0,b=0;for(let s=1;s<o.length;s++){const x=o[s].trim();if(!x)continue;const L=ve(x),A=L[t]?.trim(),V=L[n]?.trim();if(!A||!V){m++;continue}if(l.has(A.toLowerCase())){b++;continue}let C="word";if(i!==-1&&L[i]){const P=L[i].toLowerCase().trim();["phrasal","phrasal verb","phrasal-verb"].includes(P)?C="phrasal":["expression","expresion","expresión"].includes(P)?C="expression":["connector","conector"].includes(P)?C="connector":["word","palabra"].includes(P)&&(C="word")}const z={id:Date.now()+Math.random(),word:A,meaning:V,type:C,category:r!==-1&&L[r]?.trim()||null,example:d!==-1&&L[d]?.trim()||"",emotion:c!==-1&&L[c]?.trim()||"",image:"",remembered:!1,createdAt:Date.now(),lastReviewedAt:null,reviewCount:0,correctCount:0,incorrectCount:0,nextReviewAt:Date.now(),difficulty:0};g.push(z),l.add(A.toLowerCase()),y++}return localStorage.setItem(Y,JSON.stringify(g)),{success:!0,imported:y,skipped:m,duplicates:b}}catch(o){return{success:!1,error:o.message}}}function ve(e){const o=[];let a="",t=!1;for(let n=0;n<e.length;n++){const i=e[n];i==='"'?t=!t:(i===","||i===";")&&!t?(o.push(a.trim()),a=""):a+=i}return o.push(a.trim()),o}function Le(e,o=null){const a=j(),t=e.toLowerCase().trim();return a.some(n=>n.word.toLowerCase().trim()===t&&n.id!==o)}function X(){const e=localStorage.getItem(Te);return e?JSON.parse(e):{theme:"dark",language:"es",showExampleInReview:!0,autoPlayAudio:!1}}function qe(e){localStorage.setItem(Te,JSON.stringify(e))}const Ie="emowords_gamification";function ne(){const e=localStorage.getItem(Ie);return e?JSON.parse(e):{streak:0,lastStudyDate:null,maxStreak:0,dailyGoal:{date:new Date().toLocaleDateString(),count:0,target:20},totalXp:0,level:1,perfectSessions:0,totalReviews:0}}function de(e){localStorage.setItem(Ie,JSON.stringify(e))}function re(){const e=ne(),o=new Date().toLocaleDateString();if(e.dailyGoal.date!==o){e.dailyGoal={date:o,count:0,target:e.dailyGoal.target||20};const a=new Date;a.setDate(a.getDate()-1),e.lastStudyDate!==a.toLocaleDateString()&&e.lastStudyDate!==o&&(e.streak>0,e.streak=0),de(e)}return e}function fe(e=1,o={}){const a=ne(),t=new Date().toLocaleDateString(),n=a.level;if(a.dailyGoal.date!==t&&(a.dailyGoal={date:t,count:0,target:a.dailyGoal.target||20}),a.dailyGoal.count+=e,a.totalReviews=(a.totalReviews||0)+e,o.perfectSession&&(a.perfectSessions=(a.perfectSessions||0)+1),a.lastStudyDate!==t){const r=new Date;r.setDate(r.getDate()-1);const d=r.toLocaleDateString();a.lastStudyDate===d?a.streak+=1:a.streak=1,a.streak>a.maxStreak&&(a.maxStreak=a.streak),a.lastStudyDate=t}a.totalXp+=e*10,a.level=Math.floor(Math.sqrt(a.totalXp/100))+1,de(a);const i=a.level>n;return{...a,leveledUp:i,newLevel:i?a.level:null}}function ea(e){const o=ne();o.dailyGoal.target=e,de(o)}function aa(e){const o=ne();return{totalWords:e?.total||0,masteredWords:e?.mastered||0,wordsByType:e?.byType||{word:0,phrasal:0,expression:0,connector:0},maxStreak:o.maxStreak||0,streak:o.streak||0,totalReviews:o.totalReviews||0,perfectSessions:o.perfectSessions||0,level:o.level||1,studiedAtNight:!1,studiedEarly:!1}}const Ce="emowords_achievements",K={first_word:{id:"first_word",name:"Primera Palabra",description:"Añade tu primera palabra",icon:"fa-seedling",category:"vocabulary",xpReward:10,condition:e=>e.totalWords>=1},collector_10:{id:"collector_10",name:"Coleccionista",description:"Alcanza 10 palabras en tu vocabulario",icon:"fa-layer-group",category:"vocabulary",xpReward:25,condition:e=>e.totalWords>=10},collector_50:{id:"collector_50",name:"Bibliotecario",description:"Alcanza 50 palabras en tu vocabulario",icon:"fa-book",category:"vocabulary",xpReward:50,condition:e=>e.totalWords>=50},collector_100:{id:"collector_100",name:"Erudito",description:"Alcanza 100 palabras en tu vocabulario",icon:"fa-graduation-cap",category:"vocabulary",xpReward:100,condition:e=>e.totalWords>=100},collector_500:{id:"collector_500",name:"Maestro del Léxico",description:"Alcanza 500 palabras en tu vocabulario",icon:"fa-crown",category:"vocabulary",xpReward:250,condition:e=>e.totalWords>=500},streak_3:{id:"streak_3",name:"En Racha",description:"Mantén una racha de 3 días",icon:"fa-fire",category:"streak",xpReward:30,condition:e=>e.maxStreak>=3},streak_7:{id:"streak_7",name:"Semana Perfecta",description:"Mantén una racha de 7 días",icon:"fa-fire-flame-curved",category:"streak",xpReward:70,condition:e=>e.maxStreak>=7},streak_30:{id:"streak_30",name:"Mes de Fuego",description:"Mantén una racha de 30 días",icon:"fa-meteor",category:"streak",xpReward:200,condition:e=>e.maxStreak>=30},streak_100:{id:"streak_100",name:"Imparable",description:"Mantén una racha de 100 días",icon:"fa-dragon",category:"streak",xpReward:500,condition:e=>e.maxStreak>=100},first_review:{id:"first_review",name:"Primer Repaso",description:"Completa tu primera sesión de repaso",icon:"fa-play",category:"review",xpReward:15,condition:e=>e.totalReviews>=1},reviewer_50:{id:"reviewer_50",name:"Repasador",description:"Completa 50 repasos",icon:"fa-rotate",category:"review",xpReward:50,condition:e=>e.totalReviews>=50},reviewer_200:{id:"reviewer_200",name:"Experto en Repasos",description:"Completa 200 repasos",icon:"fa-brain",category:"review",xpReward:100,condition:e=>e.totalReviews>=200},perfect_session:{id:"perfect_session",name:"Sesión Perfecta",description:"Completa una sesión sin errores (mín. 5 palabras)",icon:"fa-star",category:"review",xpReward:40,condition:e=>e.perfectSessions>=1},first_master:{id:"first_master",name:"Primera Maestría",description:"Domina completamente tu primera palabra",icon:"fa-gem",category:"mastery",xpReward:30,condition:e=>e.masteredWords>=1},master_10:{id:"master_10",name:"Dominador",description:"Domina 10 palabras",icon:"fa-trophy",category:"mastery",xpReward:75,condition:e=>e.masteredWords>=10},master_50:{id:"master_50",name:"Gran Maestro",description:"Domina 50 palabras",icon:"fa-medal",category:"mastery",xpReward:200,condition:e=>e.masteredWords>=50},level_5:{id:"level_5",name:"Aprendiz Dedicado",description:"Alcanza el nivel 5",icon:"fa-arrow-up",category:"level",xpReward:50,condition:e=>e.level>=5},level_10:{id:"level_10",name:"Estudiante Avanzado",description:"Alcanza el nivel 10",icon:"fa-ranking-star",category:"level",xpReward:100,condition:e=>e.level>=10},level_25:{id:"level_25",name:"Leyenda del Vocabulario",description:"Alcanza el nivel 25",icon:"fa-chess-king",category:"level",xpReward:300,condition:e=>e.level>=25},variety_master:{id:"variety_master",name:"Variedad",description:"Tiene al menos 5 de cada tipo (palabra, phrasal, expresión, conector)",icon:"fa-shapes",category:"special",xpReward:60,condition:e=>e.wordsByType.word>=5&&e.wordsByType.phrasal>=5&&e.wordsByType.expression>=5&&e.wordsByType.connector>=5},night_owl:{id:"night_owl",name:"Búho Nocturno",description:"Estudia después de medianoche",icon:"fa-moon",category:"special",xpReward:20,condition:e=>e.studiedAtNight},early_bird:{id:"early_bird",name:"Madrugador",description:"Estudia antes de las 7am",icon:"fa-sun",category:"special",xpReward:20,condition:e=>e.studiedEarly}};function ie(){const e=localStorage.getItem(Ce);return e?JSON.parse(e):[]}function Se(e){localStorage.setItem(Ce,JSON.stringify(e))}function oa(e){const o=ie(),a=[];return Object.values(K).forEach(t=>{o.includes(t.id)||t.condition(e)&&(o.push(t.id),a.push(t))}),a.length>0&&Se(o),a}function Ae(){const e=ie();return Object.values(K).map(o=>({...o,unlocked:e.includes(o.id)}))}function Pe(){const e=Object.keys(K).length,o=ie().length;return{total:e,unlocked:o,percent:Math.round(o/e*100)}}function ta(){const e=new Date().getHours(),o={studiedAtNight:e>=0&&e<5,studiedEarly:e>=5&&e<7},a=ie(),t=[];return o.studiedAtNight&&!a.includes("night_owl")&&(a.push("night_owl"),t.push(K.night_owl)),o.studiedEarly&&!a.includes("early_bird")&&(a.push("early_bird"),t.push(K.early_bird)),t.length>0&&Se(a),t}const he="toast-container";function na(){let e=document.getElementById(he);return e||(e=document.createElement("div"),e.id=he,e.className="toast-container",document.body.appendChild(e)),e}function I(e,o,a="info",t=4e3){const n=na(),i=document.createElement("div"),r={info:"fa-circle-info",success:"fa-circle-check",warning:"fa-triangle-exclamation",error:"fa-circle-xmark"};i.className=`toast ${a}`,i.innerHTML=`
  ...
  `,a.querySelector(".speak-btn").addEventListener("click",c=>{c.stopPropagation(),W(e.word)}),a.querySelector(".toggle").addEventListener("click",()=>{e.remembered=!e.remembered,le(e),o()}),a.querySelector(".delete").addEventListener("click",()=>{confirm(`¿Eliminar "${e.word}"?`)&&(Oe(e.id),o())}),a.querySelector(".edit-btn").addEventListener("click",()=>{ua(e,o)}),a}function ua(e,o){document.querySelector(".edit-modal")?.remove();const a=document.createElement("div");a.className="edit-modal",a.innerHTML=`
  ...
  `,document.body.appendChild(a),requestAnimationFrame(()=>{a.classList.add("active")});const t=()=>{a.classList.remove("active"),setTimeout(()=>a.remove(),300)};a.querySelector(".modal-overlay").addEventListener("click",t),a.querySelector(".modal-close").addEventListener("click",t),a.querySelector(".btn-cancel").addEventListener("click",t);let n=null;a.querySelectorAll(".image-tab").forEach(c=>{c.addEventListener("click",()=>{a.querySelectorAll(".image-tab").forEach(l=>l.classList.remove("active")),c.classList.add("active");const g=c.dataset.tab;a.querySelector("#tab-url").style.display=g==="url"?"block":"none",a.querySelector("#tab-upload").style.display=g==="upload"?"block":"none"})});const i=a.querySelector("#edit-image-file"),r=a.querySelector("#edit-dropzone");if(i&&r){let c=function(g){const l=new FileReader;l.onload=y=>{n=y.target.result,r.classList.add("has-file");const m=r.querySelector(".dropzone-text"),b=r.querySelector(".dropzone-subtext");m&&(m.textContent=g.name),b&&(b.textContent=`${(g.size/1024).toFixed(1)} KB`);let s=a.querySelector(".image-preview-mini");s||(s=document.createElement("div"),s.className="image-preview-mini",r.parentElement.after(s)),s.innerHTML=`<img src="${n}" alt="Preview" />`},l.readAsDataURL(g)};var d=c;["dragenter","dragover"].forEach(g=>{r.addEventListener(g,l=>{l.preventDefault(),r.classList.add("dragover")})}),["dragleave","drop"].forEach(g=>{r.addEventListener(g,l=>{l.preventDefault(),r.classList.remove("dragover")})}),r.addEventListener("drop",g=>{const l=g.dataTransfer.files[0];l&&l.type.startsWith("image/")&&c(l)}),i.addEventListener("change",g=>{const l=g.target.files[0];l&&c(l)})}a.querySelector(".edit-form").addEventListener("submit",c=>{c.preventDefault();const g=document.getElementById("edit-word").value.trim(),l=a.querySelector(".edit-feedback");if(Le(g,e.id)){l.textContent=`La palabra "${g}" ya existe.`,l.style.display="block";return}e.word=g,e.meaning=document.getElementById("edit-meaning").value.trim(),e.type=document.getElementById("edit-type").value,e.category=document.getElementById("edit-category").value.trim()||null,e.emotion=document.getElementById("edit-emotion").value.trim(),e.example=document.getElementById("edit-example").value.trim(),n?e.image=n:e.image=document.getElementById("edit-image").value.trim(),le(e),t(),o()}),document.addEventListener("keydown",function c(g){g.key==="Escape"&&(t(),document.removeEventListener("keydown",c))})}function G(e){return e?e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;"):""}function ga(e){switch(e){case"word":return'<i class="fa-solid fa-font"></i> Palabra';case"phrasal":return'<i class="fa-solid fa-link"></i> Phrasal Verb';case"expression":return'<i class="fa-solid fa-comment"></i> Expresión';case"connector":return'<i class="fa-solid fa-arrows-left-right"></i> Conector';default:return'<i class="fa-solid fa-file"></i> Otro'}}function se(e){const o=ce(),a=Ee(),t=re(),n=24,i=2*Math.PI*n,r=Math.min(t.dailyGoal.count/t.dailyGoal.target,1),d=i-r*i,c=o.total>0;e.innerHTML=`
  ...
  `;const g=document.getElementById("word-list"),l=document.getElementById("filter-mastery"),y=document.getElementById("filter-type"),m=document.getElementById("filter-category"),b=document.getElementById("sort-by"),s=document.getElementById("search-input"),x=document.getElementById("clear-search"),L=document.getElementById("results-info"),A=document.getElementById("export-btn"),V=document.getElementById("import-btn"),C=document.getElementById("add-packs-btn"),z=document.getElementById("import-file");function P(){g.innerHTML="";let p=s.value.trim()?Ge(s.value.trim()):j();const k=Date.now();p=p.filter(v=>{let w=!0;l.value!=="all"&&(l.value==="due"?w=!v.nextReviewAt||v.nextReviewAt<=k:w=ke(v)===l.value);const E=y.value==="all"||v.type===y.value,u=m.value==="all"||v.category===m.value;return w&&E&&u}),p=_e(p,b.value);const T=j().length;if(s.value.trim()?(L.innerHTML=`<span class="results-count">${p.length} resultados</span> para "<strong>${s.value}</strong>"`,L.style.display="block"):p.length!==T?(L.innerHTML=`<span class="results-count">${p.length} de ${T}</span> palabras`,L.style.display="block"):L.style.display="none",p.length===0){g.innerHTML=`
          ...
          ${s.value.trim()?"":`
            ...
            <div class="starter-packs-section">
              ...
              <div class="starter-packs-grid">
                ${D.map(w=>`
                  <div class="pack-card" data-pack-id="${w.id}">
      ...
      `;const v=g.querySelector("#import-packs-btn");if(v){let w=new Set;const E=()=>{const u=w.size;if(v.disabled=u===0,u===0)v.textContent="Selecciona packs",v.classList.remove("active");else{let q=0;w.forEach(R=>{const $=D.find(Q=>Q.id===R);$&&(q+=$.words.length)}),v.innerHTML=`<i class="fa-solid fa-download"></i> Añadir ${u} pack${u>1?"s":""} (${q} palabras)`,v.classList.add("active")}};g.querySelectorAll(".pack-card").forEach(u=>{u.addEventListener("click",()=>{const q=u.dataset.packId;w.has(q)?(w.delete(q),u.classList.remove("selected")):(w.add(q),u.classList.add("selected")),E()})}),v.addEventListener("click",()=>{if(w.size!==0&&confirm(`¿Añadir ${w.size} packs a tu vocabulario?`)){let u=[];w.forEach($=>{const Q=D.find(Be=>Be.id===$);Q&&(u=u.concat(Q.words))});const q=JSON.stringify({words:u}),R=oe(q);R.success?(I("Packs añadidos",`¡Genial! Se han añadido ${R.imported} palabras nuevas.`,"success"),se(e)):I("Error","Hubo un problema al cargar los packs.","error")}})}}else p.forEach(v=>{g.appendChild(ma(v,P))})}l.addEventListener("change",P),y.addEventListener("change",P),m.addEventListener("change",P),b.addEventListener("change",P);let F;s.addEventListener("input",()=>{x.style.display=s.value?"flex":"none",clearTimeout(F),F=setTimeout(P,300)}),x.addEventListener("click",()=>{s.value="",x.style.display="none",P()}),A.addEventListener("click",()=>{const p=Qe(),k=new Blob([p],{type:"application/json"}),T=URL.createObjectURL(k),v=document.createElement("a");v.href=T,v.download=`emowords-backup-${new Date().toISOString().split("T")[0]}.json`,v.click(),URL.revokeObjectURL(T)}),C&&C.addEventListener("click",()=>{h()}),V.addEventListener("click",()=>{z.click()}),z.addEventListener("change",p=>{const k=p.target.files[0];if(!k)return;const T=k.name.toLowerCase().endsWith(".csv"),v=new FileReader;v.onload=w=>{const E=w.target.result;let u;if(T)if(u=Ze(E),u.success){let q=`${u.imported} palabras importadas.`;u.duplicates>0&&(q+=` ${u.duplicates} duplicadas omitidas.`),u.skipped>0&&(q+=` ${u.skipped} con errores.`),I("Importación CSV completada",q,"success"),setTimeout(()=>location.reload(),1500)}else I("Error de importación CSV",u.error,"error");else u=oe(E),u.success?(I("Importación completada",`${u.imported} palabras importadas.`,"success"),setTimeout(()=>location.reload(),1500)):I("Error de importación",u.error,"error")},v.readAsText(k),p.target.value=""});const B=document.getElementById("goal-settings-btn");B&&B.addEventListener("click",()=>{f()});const J=document.getElementById("explore-packs-btn");J&&J.addEventListener("click",()=>{h()});function h(){document.querySelector(".packs-modal")?.remove();let p=new Set;const k=document.createElement("div");k.className="packs-modal edit-modal",k.innerHTML=`
      <div class="modal-overlay"></div>
      <div class="modal-content" style="max-width: 700px;">
        ...
        <div class="packs-modal-grid">
          ${D.map(E=>{const u=localStorage.getItem("pack_"+E.id);return`
            <div class="pack-card-modal ${u?"added":""}" data-pack-id="${E.id}">
    ...
    `,document.body.appendChild(k),requestAnimationFrame(()=>{k.classList.add("active")});const T=()=>{k.classList.remove("active"),setTimeout(()=>k.remove(),300)};k.querySelector(".modal-overlay").addEventListener("click",T),k.querySelector(".modal-close").addEventListener("click",T),k.querySelector(".btn-cancel").addEventListener("click",T);const v=k.querySelector(".btn-import-packs"),w=()=>{const E=p.size;if(v.disabled=E===0,E===0)v.textContent="Selecciona packs";else{let u=0;p.forEach(q=>{const R=D.find($=>$.id===q);R&&(u+=R.words.length)}),v.innerHTML=`<i class="fa-solid fa-download"></i> Añadir ${E} pack${E>1?"s":""} (${u} palabras)`}};k.querySelectorAll(".pack-card-modal").forEach(E=>{E.classList.contains("added")||E.addEventListener("click",()=>{const u=E.dataset.packId;p.has(u)?(p.delete(u),E.classList.remove("selected")):(p.add(u),E.classList.add("selected")),w()})}),v.addEventListener("click",()=>{if(p.size===0)return;let E=[];p.forEach(q=>{const R=D.find($=>$.id===q);R&&(E=[...E,...R.words],localStorage.setItem("pack_"+q,"true"))});const u=oe(JSON.stringify({words:E}));u.success?(I("¡Packs importados!",`Se añadieron ${u.imported} palabras.`,"success"),T(),setTimeout(()=>location.reload(),1e3)):I("Error",u.error,"error")})}function f(){document.querySelector(".goal-settings-modal")?.remove();const p=re().dailyGoal.target,k=[5,10,15,20,30,50],T=document.createElement("div");T.className="goal-settings-modal edit-modal",T.innerHTML=`
      <div class="modal-overlay"></div>
      <div class="modal-content" style="max-width: 400px;">
        ...
        <div class="goal-options-grid">
          ${k.map(w=>`
            <button class="goal-option ${w===p?"active":""}" data-value="${w}">
              <span class="goal-number">${w}</span>
    ...
    `,document.body.appendChild(T),requestAnimationFrame(()=>{T.classList.add("active")});const v=()=>{T.classList.remove("active"),setTimeout(()=>T.remove(),300)};T.querySelector(".modal-overlay").addEventListener("click",v),T.querySelector(".modal-close").addEventListener("click",v),T.querySelector(".btn-cancel").addEventListener("click",v),T.querySelectorAll(".goal-option").forEach(w=>{w.addEventListener("click",()=>{const E=parseInt(w.dataset.value);ea(E),I("Meta actualizada",`Tu nueva meta diaria es ${E} palabras.`,"success"),v(),se(e)})}),document.addEventListener("keydown",function w(E){E.key==="Escape"&&(v(),document.removeEventListener("keydown",w))})}P()}function ya(e){return 100*Math.pow(e,2)}function va(e,o){const a=100*Math.pow(o-1,2),n=100*Math.pow(o,2)-a,i=e-a;return Math.min(100,Math.max(0,i/n*100))}function fa(e,o){return e>=o?"¡Objetivo completado!":e>=o*.75?"¡Casi lo tienes!":e>=o*.5?"¡Ya vas por la mitad!":"¡Vamos a por ello!"}function ha(e){const o=Ee();e.innerHTML=`
...
Ejemplo: Mi coche se averió en la autopista y tuve que esperar 2 horas bajo la lluvia..."></textarea>
  ...
  `;const a=document.getElementById("add-word-form"),t=document.getElementById("image-preview"),n=document.getElementById("preview-img"),i=document.getElementById("remove-preview"),r=document.getElementById("clear-form"),d=document.getElementById("image-data"),c=document.querySelectorAll(".image-tab"),g=document.querySelectorAll(".image-tab-content"),l=document.getElementById("upload-area"),y=document.getElementById("image-file"),m=document.getElementById("image-url"),b=document.getElementById("preview-url-btn");c.forEach(x=>{x.addEventListener("click",()=>{const L=x.dataset.tab;c.forEach(A=>A.classList.remove("active")),x.classList.add("active"),g.forEach(A=>{A.classList.remove("active"),A.id===`${L}-content`&&A.classList.add("active")})})}),l.addEventListener("click",()=>{y.click()}),l.addEventListener("dragover",x=>{x.preventDefault(),l.classList.add("dragover")}),l.addEventListener("dragleave",()=>{l.classList.remove("dragover")}),l.addEventListener("drop",x=>{x.preventDefault(),l.classList.remove("dragover");const L=x.dataTransfer.files;L.length>0&&s(L[0])}),y.addEventListener("change",x=>{x.target.files.length>0&&s(x.target.files[0])});function s(x){if(!x.type.startsWith("image/")){I("Archivo inválido","Por favor selecciona un archivo de imagen.","error");return}const L=2*1024*1024;if(x.size>L){I("Imagen muy grande","La imagen debe ser menor a 2MB.","error");return}const A=new FileReader;A.onload=V=>{const C=V.target.result;n.src=C,t.style.display="block",d.value=C,l.classList.add("has-file"),I("Imagen cargada","La imagen se ha cargado correctamente.","success")},A.onerror=()=>{I("Error","No se pudo leer la imagen.","error")},A.readAsDataURL(x)}b.addEventListener("click",()=>{const x=m.value.trim();x&&(n.src=x,t.style.display="block",d.value=x,n.onerror=()=>{t.style.display="none",d.value="",I("Error de imagen","No se pudo cargar la imagen. Verifica la URL.","warning")})}),m.addEventListener("keypress",x=>{x.key==="Enter"&&(x.preventDefault(),b.click())}),i.addEventListener("click",()=>{m.value="",y.value="",d.value="",t.style.display="none",n.src="",l.classList.remove("has-file")}),r.addEventListener("click",()=>{a.reset(),t.style.display="none",n.src="",d.value="",l.classList.remove("has-file")}),a.addEventListener("submit",x=>{x.preventDefault();const L=document.getElementById("word").value.trim(),A=document.getElementById("meaning").value.trim(),V=document.getElementById("type").value,C=document.getElementById("category").value.trim(),z=document.getElementById("emotion").value.trim(),P=document.getElementById("example").value.trim(),F=d.value.trim();if(!L||!A){I("Faltan datos","Por favor completa al menos la palabra y su significado.","error");return}if(Le(L)){I("Palabra duplicada",`La palabra "${L}" ya existe en tu vocabulario.`,"error");return}const B={id:Date.now(),word:L,meaning:A,type:V,category:C||null,emotion:z,example:P,image:F,remembered:!1};We(B),a.reset(),t.style.display="none",n.src="",d.value="",l.classList.remove("has-file"),I("¡Guardado!",`"${L}" se ha añadido correctamente.`,"success"),document.getElementById("word").focus()})}const wa="modulepreload",ba=function(e){return"/emowords/"+e},be={},xa=function(o,a,t){let n=Promise.resolve();if(a&&a.length>0){let g=function(l){return Promise.all(l.map(y=>Promise.resolve(y).then(m=>({status:"fulfilled",value:m}),m=>({status:"rejected",reason:m}))))};var r=g;document.getElementsByTagName("link");const d=document.querySelector("meta[property=csp-nonce]"),c=d?.nonce||d?.getAttribute("nonce");n=g(a.map(l=>{if(l=ba(l),l in be)return;be[l]=!0;const y=l.endsWith(".css"),m=y?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${l}"]${m}`))return;const b=document.createElement("link");if(b.rel=y?"stylesheet":wa,y||(b.as="script"),b.crossOrigin="",b.href=l,c&&b.setAttribute("nonce",c),document.head.appendChild(b),y)return new Promise((s,x)=>{b.addEventListener("load",s),b.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${l}`)))})}))}function i(d){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=d,window.dispatchEvent(c),!c.defaultPrevented)throw d}return n.then(d=>{for(const c of d||[])c.status==="rejected"&&i(c.reason);return o().catch(i)})};function Ta(e){let o=null,a=null,t=!1,n=[],i={correct:0,incorrect:0,xp:0},r=new Map;const d=2;n=Xe(),B(n);function c(){e.innerHTML="",o||g()}function g(){const h=n.length;e.innerHTML=`
    ...
    `,e.querySelectorAll(".mode-card").forEach(p=>{p.addEventListener("click",()=>{if(o=p.dataset.mode,console.log(`Starting mode: ${o} with queue size: ${n.length}`),n.length===0){I("Sin palabras","No hay palabras pendientes para repasar ahora.","info"),z(e);return}c(),l(),s()})});const f=e.querySelector("#explore-packs-review-btn");f&&f.addEventListener("click",()=>{document.querySelector("[data-view=home]").click(),setTimeout(()=>{const p=document.getElementById("explore-packs-btn");p&&p.click()},100)})}function l(){if(e.querySelector(".review-header"))return;const h=document.createElement("div");h.className="review-header",h.innerHTML=`
     ...
     `,e.insertBefore(h,e.firstChild),document.getElementById("exit-mode").addEventListener("click",k=>{k.preventDefault(),i.correct>0||i.incorrect>0?confirm("¿Salir del modo repaso? Tu progreso se perderá.")&&m():m()});const p=document.createElement("div");p.id="active-content",p.className="review-container",e.appendChild(p)}function y(){const h=document.querySelector("#stat-queue .val"),f=document.querySelector("#stat-correct .val"),p=document.querySelector("#stat-xp .val");h&&(h.textContent=n.length),f&&(f.textContent=i.correct),p&&(p.textContent=`${i.xp} XP`)}function m(){o=null,c()}function b(){return n.shift()||null}function s(){if(!o){c();return}const h=document.getElementById("active-content");if(!h){c();return}if(a=b(),!a){z(h);return}let f=o;if(o==="mixed"){const p=["flashcard","quiz","typing","listening"];f=p[Math.floor(Math.random()*p.length)]}switch(f){case"flashcard":x(h);break;case"quiz":L(h);break;case"typing":A(h);break;case"listening":V(h);break}}function x(h){t=!1,a.reviewCount,h.innerHTML=`
      <div class="review-card" id="review-card">
        <div class="review-card-inner">
           <div class="review-meta">
              <span class="tag type-tag">${J(a.type)}</span>
    ...
    `,document.getElementById("review-speak-btn").addEventListener("click",v=>{v.stopPropagation(),W(a.word)});const f=document.getElementById("show-answer"),p=document.getElementById("review-answer"),k=document.getElementById("remembered-btn"),T=document.getElementById("forgotten-btn");f.addEventListener("click",()=>{t=!0,p.style.display="block",f.style.display="none",k.disabled=!1,T.disabled=!1,p.classList.add("fade-in")}),k.addEventListener("click",()=>C(!0)),T.addEventListener("click",()=>C(!1)),F(v=>{v==="Space"&&!t&&f.click(),v==="ArrowRight"&&t&&C(!0),v==="ArrowLeft"&&t&&C(!1)})}function L(h){const p=j().filter(w=>w.id!==a.id).sort(()=>.5-Math.random()).slice(0,3),k=[a,...p];B(k),h.innerHTML=`
      <div class="quiz-container">
         ...
         <div class="quiz-options">
            ${k.map(w=>`
                <button class="quiz-option" data-id="${w.id}">
                    ${w.meaning}
                </button>
            `).join("")}
         </div>
      </div>
    `,document.getElementById("quiz-speak-btn").addEventListener("click",()=>W(a.word));const T=h.querySelectorAll(".quiz-option");let v=!1;T.forEach(w=>{w.addEventListener("click",()=>{if(v)return;v=!0,String(w.dataset.id)===String(a.id)?(w.classList.add("correct"),setTimeout(()=>C(!0),800)):(w.classList.add("wrong"),T.forEach(q=>{String(q.dataset.id)===String(a.id)&&q.classList.add("correct")}),setTimeout(()=>C(!1),1500))})})}function A(h){h.innerHTML=`
    ...
    `,setTimeout(()=>document.getElementById("type-input").focus(),100);const f=document.getElementById("type-input"),p=document.getElementById("check-btn"),k=document.getElementById("give-up-btn");function T(){f.value.trim().toLowerCase()===a.word.toLowerCase()?(f.classList.add("correct"),p.innerHTML='<i class="fa-solid fa-check"></i> Correcto',W(a.word),setTimeout(()=>C(!0),1e3)):(f.classList.add("wrong"),W("Incorrect","en-US"),setTimeout(()=>f.classList.remove("wrong"),500))}p.addEventListener("click",T),f.addEventListener("keydown",v=>{v.key==="Enter"&&T()}),k.addEventListener("click",()=>{f.value=a.word,f.classList.add("wrong"),W(a.word),setTimeout(()=>C(!1),2e3)})}function V(h){const p=j().filter(u=>u.id!==a.id).sort(()=>.5-Math.random()).slice(0,3),k=[a,...p];B(k),h.innerHTML=`
      <div class="quiz-container">
         <div class="quiz-question">
    ...
    `;const T=document.getElementById("listen-icon"),v=()=>{T.style.transform="scale(0.9)",setTimeout(()=>T.style.transform="scale(1)",150),W(a.word)};T.addEventListener("click",v),setTimeout(v,500);const w=h.querySelectorAll(".quiz-option");let E=!1;w.forEach(u=>{u.addEventListener("click",()=>{if(E)return;E=!0,String(u.dataset.id)===String(a.id)?(u.classList.add("correct"),setTimeout(()=>C(!0),800)):(u.classList.add("wrong"),w.forEach($=>{String($.dataset.id)===String(a.id)&&$.classList.add("correct")}),setTimeout(()=>C(!1),1500))})})}function C(h){try{if(!a)return;if(Je(a.id,h),h){i.correct++,i.xp+=10;try{fe(1)}catch(f){console.error(f)}r.delete(a.id)}else{i.incorrect++;const f=r.get(a.id)||0;f<d&&(r.set(a.id,f+1),n.push(a))}a=null,y(),s()}catch(f){console.error("Error in handleResult:",f)}}function z(h){try{const f=i.incorrect===0&&i.correct>=5,p=fe(0,{perfectSession:f}),k=document.querySelector(".review-header");k&&(k.style.display="none");let T;try{T=re()}catch{T={streak:0,dailyGoal:{count:0,target:20}}}const v=ce(),w=aa(v),E=oa(w);la(),h.innerHTML=`
          <div class="empty-review-state">
            ...
            <div class="streak-mini" style="margin: 1.5rem 0; padding: 1rem; background: #fffbeb; border-radius: 8px; border: 1px solid #fcd34d;">
                <p style="color: #b45309; font-weight: bold;"><i class="fa-solid fa-fire"></i> Racha: ${T.streak} días</p>
                <p style="font-size: 0.9rem; color: #92400e;">Meta diaria: ${T.dailyGoal.count} / ${T.dailyGoal.target}</p>
            </div>
    
            <button class="add-word-btn" id="finish-btn">Volver al inicio</button>
          </div>
        `,p.leveledUp&&setTimeout(()=>{ra(p.newLevel)},500),E.length>0&&setTimeout(()=>{je(E)},p.leveledUp?5500:1e3),window.confetti||window.canvasConfetti?(window.confetti||window.canvasConfetti)({particleCount:100,spread:70,origin:{y:.6}}):xa(()=>import("./confetti.module-C2jkTI5u.js"),[]).then(q=>{const R=q.default;R({particleCount:100,spread:70,origin:{y:.6}})}).catch(q=>console.log("Confetti not found",q));const u=document.getElementById("finish-btn");u&&u.addEventListener("click",()=>{o=null;const q=document.querySelector('[data-view="home"]');q?q.click():c()})}catch(f){console.error("Error in renderSummary:",f),h.innerHTML='<p class="error">Error al mostrar resumen. <button onclick="location.reload()">Recargar</button></p>'}}let P=null;function F(h){P&&P();const f=p=>{document.getElementById("active-content")&&document.activeElement.tagName!=="INPUT"&&h(p.code)};document.addEventListener("keydown",f),P=()=>document.removeEventListener("keydown",f),window._reviewCleanup=P}function B(h){for(let f=h.length-1;f>0;f--){const p=Math.floor(Math.random()*(f+1));[h[f],h[p]]=[h[p],h[f]]}return h}function J(h){return{word:"Palabra",phrasal:"Phrasal Verb",expression:"Expresión",connector:"Conector"}[h]||"Otro"}c()}function Ea(e){const o=j(),a=ce();if(e.innerHTML="",e.className="stats-view animate__animated animate__fadeIn",a.total===0){qa(e);return}let t=0,n={master:0,guru:0,apprentice:0,new:0};o.forEach(b=>{const s=b.correctCount||0;s>=10?(t+=100,n.master++):s>=5?(t+=75,n.guru++):s>=2?(t+=40,n.apprentice++):(t+=10,n.new++)});const i=a.total>0?Math.round(t/a.total):0,r=Ia(o),d=Ca(o),c=Pe(),l=Ae().slice(0,8),y=Pa(o,r),m=`
    <!-- Hero Section: Grade + 3 KPIs -->
    <section class="stats-hero">
      <div class="hero-grade ${Aa(i)}">
        <span class="grade-letter">${Sa(i)}</span>
      </div>
      <div class="hero-kpis">
        <div class="kpi-item">
          ...
          <div class="kpi-mini-chart">${Ra(r)}</div>
        </div>
        <div class="kpi-item">
          ...
          <div class="kpi-ring" style="--percent: ${i}">
      ...
      <div class="stats-column-side">
        ...
        <div class="stats-card levels-card">
          ...
          <div class="levels-list">
            <div class="level-row master">
              ...
              <div class="level-bar"><div class="bar-fill" style="width: ${Z(n.master,a.total)}%"></div></div>
              <span class="level-count">${n.master}</span>
            </div>
            <div class="level-row guru">
              <span class="level-name">Experto</span>
              <div class="level-bar"><div class="bar-fill" style="width: ${Z(n.guru,a.total)}%"></div></div>
              <span class="level-count">${n.guru}</span>
            </div>
            <div class="level-row apprentice">
              <span class="level-name">Aprendiz</span>
              <div class="level-bar"><div class="bar-fill" style="width: ${Z(n.apprentice,a.total)}%"></div></div>
              <span class="level-count">${n.apprentice}</span>
            </div>
            <div class="level-row new">
              <span class="level-name">Nuevo</span>
              <div class="level-bar"><div class="bar-fill" style="width: ${Z(n.new,a.total)}%"></div></div>
              <span class="level-count">${n.new}</span>
            </div>
          </div>
        </div>
          ...
          <div class="struggling-list">
            ${d.slice(0,4).map(b=>`
              <div class="struggling-item">
                ...
                <span class="error-count">${b.incorrectCount} errores</span>
              </div>
            `).join("")}
        ...
        <div class="stats-card achievements-card">
          ...
          <div class="achievements-grid-mini">
            ${l.map(b=>`
              <div class="achievement-icon ${b.unlocked?"unlocked":"locked"}" title="${b.name}: ${b.description}">
    ...
    `:""}
  `;e.innerHTML=m,ka(e)}function ka(e){e.querySelectorAll(".collapse-btn").forEach(a=>{a.addEventListener("click",()=>{a.closest(".stats-card").classList.toggle("collapsed");const n=a.querySelector("i");n.classList.toggle("fa-chevron-up"),n.classList.toggle("fa-chevron-down")})});const o=e.querySelector("#view-all-achievements");o&&o.addEventListener("click",()=>{La()})}function La(){const e=Ae(),o=Pe(),a=document.createElement("div");a.className="achievements-modal animate__animated animate__fadeIn",a.innerHTML=`
      ...
      <div class="achievements-grid">
        ${e.map(i=>`
          <div class="achievement-card ${i.unlocked?"unlocked":"locked"}">
            ...
            ${i.unlocked?`<span class="achievement-reward">+${i.xpReward} XP</span>`:""}
          </div>
        `).join("")}
      </div>
    </div>
  `,document.body.appendChild(a),document.body.style.overflow="hidden";const t=()=>{a.classList.remove("animate__fadeIn"),a.classList.add("animate__fadeOut"),setTimeout(()=>{a.remove(),document.body.style.overflow=""},200)};a.querySelector(".modal-close").addEventListener("click",t),a.querySelector(".achievements-modal-backdrop").addEventListener("click",t);const n=i=>{i.key==="Escape"&&(t(),document.removeEventListener("keydown",n))};document.addEventListener("keydown",n)}function qa(e){e.innerHTML=`
  ...
  `;const o=e.querySelector("#explore-packs-stats-btn");o&&o.addEventListener("click",()=>{document.querySelector("[data-view=home]").click(),setTimeout(()=>{const a=document.getElementById("explore-packs-btn");a&&a.click()},100)})}function Ia(e){const o=[...e].sort((i,r)=>(i.createdAt||0)-(r.createdAt||0)),a=new Map;let t=0;o.forEach(i=>{t++;const r=new Date(i.createdAt||Date.now()).toISOString().split("T")[0];a.set(r,t)});const n=Array.from(a.entries()).map(([i,r])=>({date:i,count:r}));if(n.length>0&&n.length<2){const i=new Date(n[0].date);i.setDate(i.getDate()-1),n.unshift({date:i.toISOString().split("T")[0],count:0})}return n}function Ca(e){return[...e].filter(o=>(o.incorrectCount||0)>0).sort((o,a)=>(a.incorrectCount||0)-(o.incorrectCount||0)).slice(0,5)}function Z(e,o){return o?Math.round(e/o*100):0}function Sa(e){return e>=90?"S":e>=80?"A":e>=60?"B":e>=40?"C":"D"}function Aa(e){return e>=90?"grade-s":e>=80?"grade-a":e>=60?"grade-b":e>=40?"grade-c":"grade-d"}function Pa(e,o){const a=e.length;if(a<2||o.length<2)return null;const t=new Date(o[0].date),n=new Date(o[o.length-1].date),i=Math.max(1,Math.ceil((n-t)/(1440*60*1e3))),r=a/i;return a<500?`A este ritmo alcanzarás <strong>500 palabras</strong> en ~${Math.ceil((500-a)/r)} días`:a<1e3?`A este ritmo alcanzarás <strong>1000 palabras</strong> en ~${Math.ceil((1e3-a)/r)} días`:`¡Increíble! Ya dominas <strong>${a}</strong> palabras 🎉`}function ja(e){if(!e||e.length===0)return'<div class="chart-placeholder">Sin datos suficientes</div>';const o=600,a=200,t=30,n=e[e.length-1].count;if(n===0)return"";const i=e.map((g,l)=>{const y=l/(e.length-1)*(o-2*t)+t,m=a-(g.count/n*(a-2*t)+t);return`${y},${m}`}).join(" "),r=t,d=o-t,c=`${r},${a-t} ${i} ${d},${a-t}`;return`
    <svg viewBox="0 0 ${o} ${a}" class="growth-svg" preserveAspectRatio="none">
      ...
      <polyline points="${i}" fill="none" stroke="var(--primary-400)" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" />
      <circle cx="${o-t}" cy="${a-(n/n*(a-2*t)+t)}" r="4" fill="var(--primary-400)" />
    </svg>
    <div class="chart-label">${n} palabras</div>
  `}function Ra(e){if(!e||e.length<2)return"";const o=50,a=24,t=e[e.length-1].count,n=e.map((i,r)=>{const d=r/(e.length-1)*o,c=a-i.count/t*a;return`${d},${c}`}).join(" ");return`
    <svg viewBox="0 0 ${o} ${a}" class="sparkline">
      <polyline points="${n}" fill="none" stroke="currentColor" stroke-width="1.5" />
    </svg>
  `}function $a(e){const o=new Date;o.setHours(23,59,59,999);const a=18,t=new Map;e.forEach(l=>{if(l.lastReviewedAt){const y=new Date(l.lastReviewedAt).toISOString().split("T")[0];t.set(y,(t.get(y)||0)+1)}if(l.createdAt){const y=new Date(l.createdAt).toISOString().split("T")[0];t.set(y,(t.get(y)||0)+1)}});const n=Math.max(...Array.from(t.values()),1);let i='<div class="heatmap-grid">';const r=a*7,d=new Date(o);d.setDate(o.getDate()-r);const c=d.getDay(),g=c===0?6:c-1;d.setDate(d.getDate()-g);for(let l=0;l<a;l++){i+='<div class="heatmap-week">';for(let y=0;y<7;y++){const m=new Date(d);m.setDate(d.getDate()+l*7+y);const b=m.toISOString().split("T")[0],s=t.get(b)||0,x=s===0?0:Math.min(4,Math.ceil(s/n*4)),L=m>o;i+=`<div class="heatmap-day level-${L?"future":x}" title="${b}: ${s} actividades"></div>`}i+="</div>"}return i+="</div>",i}const Re="emowords_onboarding_completed",$e="emowords_user_level",ee=[{id:"welcome",target:null,title:"¡Bienvenido a EmoWords! 👋",content:"Aprende vocabulario conectando cada palabra con tus recuerdos personales. Te guiaremos en 30 segundos.",position:"center",icon:"fa-solid fa-rocket"},{id:"level-select",target:null,title:"¿Cuál es tu nivel de inglés?",content:"Esto nos ayudará a sugerirte los packs más adecuados para ti.",position:"center",icon:"fa-solid fa-graduation-cap",type:"level-selection"},{id:"pack-select",target:null,title:"¡Empieza con ventaja! 🎁",content:"Hemos seleccionado estos packs ideales para tu nivel. Añádelos para empezar con vocabulario útil desde el primer minuto.",position:"center",type:"pack-selection",icon:"fa-solid fa-gift"},{id:"nav-add",target:'[data-view="add"]',title:"1. Añadir Vocabulario",content:"Aquí añades palabras, phrasal verbs o expresiones con su significado.",position:"bottom",icon:"fa-solid fa-plus"},{id:"nav-review",target:'[data-view="review"]',title:"2. Repasar",content:"5 modos de práctica: Flashcards, Quiz, Escritura, Listening y Mixto.",position:"bottom",icon:"fa-solid fa-graduation-cap"},{id:"nav-stats",target:'[data-view="stats"]',title:"3. Tu Progreso",content:"Visualiza tu evolución, logros y palabras que necesitan más atención.",position:"bottom",icon:"fa-solid fa-chart-line"},{id:"secret",target:null,title:"🧠 El Secreto de EmoWords",content:"Al añadir palabras, conecta cada una con un recuerdo personal o emoción. ¡Tu memoria será 10 veces más fuerte!",position:"center",icon:"fa-solid fa-heart"},{id:"ready",target:null,title:"¡Listo para aprender! 🚀",content:"Empieza añadiendo tu primera palabra o explora los packs predefinidos. ¡Las emociones graban, la repetición se olvida!",position:"center",icon:"fa-solid fa-check-circle"}],Va=[{code:"A1-A2",label:"Básico",description:"Principiante"},{code:"B1",label:"Intermedio bajo",description:"Pre-intermedio"},{code:"B2",label:"Intermedio alto",description:"Intermedio"},{code:"C1-C2",label:"Avanzado",description:"Avanzado/Nativo"}];let _=localStorage.getItem($e)||null,O=0,S=null,N=null,H=null;function Ve(){return!localStorage.getItem(Re)}function Ma(){localStorage.setItem(Re,"true")}function Da(){Ve()&&(O=0,za(),me(O))}function za(){document.querySelector(".onboarding-overlay")?.remove(),N=document.createElement("div"),N.className="onboarding-overlay",N.innerHTML=`
    <div class="onboarding-backdrop"></div>
    <div class="onboarding-spotlight"></div>
  `,document.body.appendChild(N),H=N.querySelector(".onboarding-spotlight")}function me(e){const o=ee[e];if(!o){De();return}S&&S.remove(),o.position==="center"||!o.target?H.style.display="none":H.style.display="block",S=document.createElement("div"),S.className="onboarding-tooltip";const a=e===ee.length-1,t=e===0,n=o.type==="level-selection",i=(e+1)/ee.length*100,r=n?`
  ...
  `:"";if(S.innerHTML=`
    ...
    <div class="tooltip-header-row">
      <span class="tooltip-step">${e+1} de ${ee.length}</span>
  ...
  `,o.type==="pack-selection"&&_){let l=[];_==="A1-A2"?l=D.filter(s=>s.level==="A1"||s.level==="A2"):_==="C1-C2"?l=D.filter(s=>s.level==="C1"||s.level==="C2"):l=D.filter(s=>s.level===_);const y=`
    ...
    `,m=document.createElement("div");m.innerHTML=S.innerHTML;const b=m.querySelector(".tooltip-body");b&&(b.insertAdjacentHTML("beforeend",y),S.innerHTML=m.innerHTML)}if(N.appendChild(S),o.position==="center"||!o.target)S.classList.add("centered"),Me();else{S.classList.remove("centered");const l=document.querySelector(o.target);l?Na(l,o.position):(S.classList.add("centered"),H.style.display="none")}const d=S.querySelector(".tooltip-skip"),c=S.querySelector(".tooltip-next"),g=S.querySelector(".tooltip-prev");if(d.addEventListener("click",Oa),c.addEventListener("click",Ba),g&&g.addEventListener("click",Wa),n){const l=S.querySelectorAll(".level-btn");l.forEach(y=>{y.addEventListener("click",()=>{l.forEach(m=>m.classList.remove("active")),y.classList.add("active"),_=y.dataset.level,localStorage.setItem($e,_),c.removeAttribute("disabled")})})}o.type==="pack-selection"&&S.querySelectorAll(".pack-card-mini").forEach(y=>{y.addEventListener("click",()=>{const m=y.dataset.packId,s=y.querySelector(".pack-add-btn").querySelector("i"),x=D.find(L=>L.id===m);y.classList.contains("added")?I("Pack ya añadido","Este pack ya está en tu colección","info"):x&&oe(JSON.stringify({words:x.words})).success&&(y.classList.add("added"),s.classList.remove("fa-plus"),s.classList.add("fa-check"),localStorage.setItem("pack_"+m,"true"),I("Pack añadido",`Has añadido ${x.name} a tu colección`,"success"))})}),requestAnimationFrame(()=>{S.classList.add("visible")})}function Na(e,o){const a=e.getBoundingClientRect(),t=8,n=16;H.style.top=`${a.top-t}px`,H.style.left=`${a.left-t}px`,H.style.width=`${a.width+t*2}px`,H.style.height=`${a.height+t*2}px`;const i=S.getBoundingClientRect();let r,d;switch(o){case"bottom":r=a.bottom+n,d=a.left+a.width/2-i.width/2;break;case"top":r=a.top-n-i.height,d=a.left+a.width/2-i.width/2;break;case"left":r=a.top+a.height/2-i.height/2,d=a.left-n-i.width;break;case"right":r=a.top+a.height/2-i.height/2,d=a.right+n;break;default:r=a.bottom+n,d=a.left+a.width/2-i.width/2}const c=window.innerWidth,g=window.innerHeight;d<16&&(d=16),d+i.width>c-16&&(d=c-i.width-16),r<16&&(r=16),r+i.height>g-16&&(r=g-i.height-16),S.style.position="fixed",S.style.top=`${r}px`,S.style.left=`${d}px`}function Me(){document.querySelectorAll(".onboarding-highlight").forEach(e=>{e.classList.remove("onboarding-highlight")})}function Ba(){O++,me(O)}function Wa(){O--,O<0&&(O=0),me(O)}function Oa(){De()}function De(){Ma(),S&&(S.classList.remove("visible"),setTimeout(()=>S?.remove(),200)),N&&(N.classList.add("fade-out"),setTimeout(()=>N?.remove(),300)),Me()}const M=document.getElementById("app");window.addEventListener("offline",()=>{I("Sin conexión","Estás trabajando en modo offline.","warning",5e3),document.body.classList.add("offline-mode")});window.addEventListener("online",()=>{I("Conexión restaurada","Tus cambios se guardarán correctamente.","success",3e3),document.body.classList.remove("offline-mode")});window.addEventListener("error",e=>{console.error("Global error:",e.error),I("Error inesperado","Ha ocurrido un error. Intenta recargar la página.","error",0)});window.addEventListener("unhandledrejection",e=>{console.error("Unhandled promise rejection:",e.reason)});const ze=document.querySelectorAll(".nav-link"),U=document.getElementById("theme-toggle"),xe=document.getElementById("audio-settings-btn");function Ha(){const e=X();if(e.theme)te(e.theme);else{const o=window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light";te(o)}}function te(e){if(document.documentElement.setAttribute("data-theme",e),U){const o=U.querySelector("i");o&&(e==="dark"?(o.className="fa-solid fa-sun",U.title="Cambiar a modo claro"):(o.className="fa-solid fa-moon",U.title="Cambiar a modo oscuro"))}}function Ga(){const o=document.documentElement.getAttribute("data-theme")==="dark"?"light":"dark";document.body.classList.add("theme-transitioning"),te(o);const a=X();a.theme=o,qe(a),setTimeout(()=>{document.body.classList.remove("theme-transitioning")},300)}U&&U.addEventListener("click",Ga);window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",e=>{X().theme||te(e.matches?"dark":"light")});function Ne(){const e=document.getElementById("review-badge");if(!e)return;const o=Ye();o>0?(e.textContent=o>99?"99+":o,e.style.display="flex"):e.style.display="none"}function _a(e){ze.forEach(o=>{o.dataset.view===e?o.classList.add("active"):o.classList.remove("active")})}function ue(e){window._reviewCleanup&&(window._reviewCleanup(),window._reviewCleanup=null),_a(e),M.style.opacity="0",M.style.transform="translateY(10px)",setTimeout(()=>{switch(e){case"home":se(M);break;case"add":ha(M);break;case"review":Ta(M);break;case"stats":Ea(M);break;default:M.innerHTML="<p>Vista no encontrada</p>"}window.scrollTo({top:0,left:0,behavior:"instant"}),requestAnimationFrame(()=>{M.style.opacity="1",M.style.transform="translateY(0)"}),Ne()},150)}M.style.transition="opacity 0.15s ease, transform 0.15s ease";ze.forEach(e=>{e.addEventListener("click",o=>{o.preventDefault();const a=e.dataset.view;ue(a)})});const ae=document.querySelector(".logo");ae&&ae.dataset.view&&ae.addEventListener("click",e=>{e.preventDefault(),ue(ae.dataset.view)});Ha();Fa();Ne();ue("home");setTimeout(()=>{Ve()&&Da()},500);function Fa(){xe&&xe.addEventListener("click",Ua)}function Ua(){document.querySelector(".audio-settings-modal")?.remove();const e=pe(),o=document.createElement("div");o.className="audio-settings-modal edit-modal",o.innerHTML=`
    <div class="modal-overlay"></div>
