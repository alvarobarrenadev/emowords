(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))i(s);new MutationObserver(s=>{for(const o of s)if(o.type==="childList")for(const n of o.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&i(n)}).observe(document,{childList:!0,subtree:!0});function a(s){const o={};return s.integrity&&(o.integrity=s.integrity),s.referrerPolicy&&(o.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?o.credentials="include":s.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(s){if(s.ep)return;s.ep=!0;const o=a(s);fetch(s.href,o)}})();const S="emowords_vocab",O="emowords_settings";function g(){const e=localStorage.getItem(S);return e?JSON.parse(e):[]}function V(e){const t=g(),a={...e,createdAt:Date.now(),lastReviewedAt:null,reviewCount:0,correctCount:0,incorrectCount:0,nextReviewAt:Date.now(),difficulty:0};t.push(a),localStorage.setItem(S,JSON.stringify(t))}function A(e){const t=g().map(a=>a.id===e.id?e:a);localStorage.setItem(S,JSON.stringify(t))}function U(e){const t=g().filter(a=>a.id!==e);localStorage.setItem(S,JSON.stringify(t))}function G(e){return g().find(t=>t.id===e)}function K(e){const t=g(),a=e.toLowerCase().trim();return a?t.filter(i=>i.word.toLowerCase().includes(a)||i.meaning.toLowerCase().includes(a)||i.example&&i.example.toLowerCase().includes(a)||i.emotion&&i.emotion.toLowerCase().includes(a)||i.category&&i.category.toLowerCase().includes(a)):t}function N(){const e=g(),t=new Set;return e.forEach(a=>{a.category&&t.add(a.category)}),Array.from(t).sort()}function Q(e,t="date-desc"){const a=[...e];switch(t){case"date-asc":return a.sort((i,s)=>(i.createdAt||i.id)-(s.createdAt||s.id));case"date-desc":return a.sort((i,s)=>(s.createdAt||s.id)-(i.createdAt||i.id));case"alpha-asc":return a.sort((i,s)=>i.word.localeCompare(s.word));case"alpha-desc":return a.sort((i,s)=>s.word.localeCompare(i.word));case"review-count":return a.sort((i,s)=>(s.reviewCount||0)-(i.reviewCount||0));case"difficulty":return a.sort((i,s)=>(s.difficulty||0)-(i.difficulty||0));default:return a}}function W(){const e=g(),t=e.length,a=e.filter(c=>c.remembered).length,i=t-a,s=e.reduce((c,y)=>c+(y.reviewCount||0),0),o=t>0?(s/t).toFixed(1):0,n={word:e.filter(c=>c.type==="word").length,phrasal:e.filter(c=>c.type==="phrasal").length,expression:e.filter(c=>c.type==="expression").length},u=e.reduce((c,y)=>c+(y.correctCount||0),0),l=e.reduce((c,y)=>c+(y.incorrectCount||0),0),f=u+l>0?(u/(u+l)*100).toFixed(1):0,m=Date.now(),x=e.filter(c=>!c.nextReviewAt||c.nextReviewAt<=m).length;return{total:t,remembered:a,forgotten:i,totalReviews:s,averageReviews:o,byType:n,retentionRate:f,dueForReview:x}}function Y(){const e=g(),t=Date.now(),a=e.filter(l=>!l.remembered),i=e.filter(l=>l.remembered&&(!l.nextReviewAt||l.nextReviewAt<=t)),s=e.filter(l=>!l.lastReviewedAt),o=[...a,...s,...i],n=new Set,u=o.filter(l=>n.has(l.id)?!1:(n.add(l.id),!0));return u.length>0?u:e}function Z(e,t){const a=G(e);if(a){if(a.remembered=t,a.lastReviewedAt=Date.now(),a.reviewCount=(a.reviewCount||0)+1,t){a.correctCount=(a.correctCount||0)+1,a.difficulty=Math.max(-3,(a.difficulty||0)-1);const i=1440*60*1e3,s=Math.pow(2,Math.min(a.correctCount,5));a.nextReviewAt=Date.now()+i*s}else a.incorrectCount=(a.incorrectCount||0)+1,a.difficulty=Math.min(3,(a.difficulty||0)+1),a.nextReviewAt=Date.now();return A(a),a}}function X(){const e=g(),t={version:"1.0",exportedAt:new Date().toISOString(),wordCount:e.length,words:e};return JSON.stringify(t,null,2)}function ee(e){try{const t=JSON.parse(e);if(!t.words||!Array.isArray(t.words))throw new Error("Invalid data format: missing words array");const a=g(),i=new Set(a.map(n=>n.id));let s=0,o=0;return t.words.forEach(n=>{if(!n.word||!n.meaning){o++;return}(!n.id||i.has(n.id))&&(n.id=Date.now()+Math.random()),n.type=n.type||"word",n.remembered=n.remembered||!1,n.createdAt=n.createdAt||Date.now(),a.push(n),i.add(n.id),s++}),localStorage.setItem(S,JSON.stringify(a)),{success:!0,imported:s,skipped:o}}catch(t){return{success:!1,error:t.message}}}function H(e,t=null){const a=g(),i=e.toLowerCase().trim();return a.some(s=>s.word.toLowerCase().trim()===i&&s.id!==t)}function R(){const e=localStorage.getItem(O);return e?JSON.parse(e):{theme:"dark",language:"es",showExampleInReview:!0,autoPlayAudio:!1}}function te(e){localStorage.setItem(O,JSON.stringify(e))}function ae(e,t){const a=document.createElement("div");a.className="word-card",a.dataset.wordId=e.id;const i=e.reviewCount||0,s=e.createdAt?new Date(e.createdAt).toLocaleDateString():"";return a.innerHTML=`
  ...
  `,a.querySelector(".toggle").addEventListener("click",()=>{e.remembered=!e.remembered,A(e),t()}),a.querySelector(".delete").addEventListener("click",()=>{confirm(`¿Eliminar "${e.word}"?`)&&(U(e.id),t())}),a.querySelector(".edit-btn").addEventListener("click",()=>{se(e,t)}),a}function se(e,t){document.querySelector(".edit-modal")?.remove();const a=document.createElement("div");a.className="edit-modal",a.innerHTML=`
  ...
  `;const i=document.getElementById("word-list"),s=document.getElementById("filter-status"),o=document.getElementById("filter-type"),n=document.getElementById("filter-category"),u=document.getElementById("sort-by"),l=document.getElementById("search-input"),f=document.getElementById("clear-search"),m=document.getElementById("results-info"),x=document.getElementById("export-btn"),c=document.getElementById("import-btn"),y=document.getElementById("import-file");function b(){i.innerHTML="";let d=l.value.trim()?K(l.value.trim()):g();d=d.filter(p=>{const v=s.value==="all"||s.value==="remembered"&&p.remembered||s.value==="forgotten"&&!p.remembered,k=o.value==="all"||p.type===o.value,I=n.value==="all"||p.category===n.value;return v&&k&&I}),d=Q(d,u.value);const h=g().length;l.value.trim()?(m.innerHTML=`<span class="results-count">${d.length} resultados</span> para "<strong>${l.value}</strong>"`,m.style.display="block"):d.length!==h?(m.innerHTML=`<span class="results-count">${d.length} de ${h}</span> palabras`,m.style.display="block"):m.style.display="none",d.length===0?i.innerHTML=`
      ...
      `:d.forEach(p=>{i.appendChild(ae(p,b))})}s.addEventListener("change",b),o.addEventListener("change",b),n.addEventListener("change",b),u.addEventListener("change",b);let L;l.addEventListener("input",()=>{f.style.display=l.value?"flex":"none",clearTimeout(L),L=setTimeout(b,300)}),f.addEventListener("click",()=>{l.value="",f.style.display="none",b()}),x.addEventListener("click",()=>{const d=X(),h=new Blob([d],{type:"application/json"}),p=URL.createObjectURL(h),v=document.createElement("a");v.href=p,v.download=`emowords-backup-${new Date().toISOString().split("T")[0]}.json`,v.click(),URL.revokeObjectURL(p)}),c.addEventListener("click",()=>{y.click()}),y.addEventListener("change",d=>{const h=d.target.files[0];if(!h)return;const p=new FileReader;p.onload=v=>{const k=ee(v.target.result);k.success?(E("Importación completada",`${k.imported} palabras importadas correctamente.`,"success"),setTimeout(()=>location.reload(),1500)):E("Error de importación",k.error,"error")},p.readAsText(h)}),b()}function re(e){const t=N();e.innerHTML=`
...
Ejemplo: Mi coche se averió en la autopista y tuve que esperar 2 horas bajo la lluvia..."></textarea>
  ...
  `;const a=document.getElementById("add-word-form"),i=document.getElementById("image"),s=document.getElementById("preview-image-btn"),o=document.getElementById("image-preview"),n=document.getElementById("preview-img"),u=document.getElementById("remove-preview"),l=document.getElementById("clear-form");s.addEventListener("click",()=>{const f=i.value.trim();f&&(n.src=f,o.style.display="block",n.onerror=()=>{o.style.display="none",E("Error de imagen","No se pudo cargar la imagen. Verifica la URL.","warning")})}),u.addEventListener("click",()=>{i.value="",o.style.display="none",n.src=""}),l.addEventListener("click",()=>{a.reset(),o.style.display="none",n.src=""}),a.addEventListener("submit",f=>{f.preventDefault();const m=document.getElementById("word").value.trim(),x=document.getElementById("meaning").value.trim(),c=document.getElementById("type").value,y=document.getElementById("category").value.trim(),b=document.getElementById("emotion").value.trim(),L=document.getElementById("example").value.trim(),d=document.getElementById("image").value.trim();if(!m||!x){E("Faltan datos","Por favor completa al menos la palabra y su significado.","error");return}if(H(m)){E("Palabra duplicada",`La palabra "${m}" ya existe en tu vocabulario.`,"error");return}const h={id:Date.now(),word:m,meaning:x,type:c,category:y||null,emotion:b,example:L,image:d,remembered:!1};V(h),a.reset(),o.style.display="none",n.src="",E("¡Guardado!",`"${m}" se ha añadido correctamente.`,"success"),document.getElementById("word").focus()})}function le(e){let t=null,a=!1,i=[],s={correct:0,incorrect:0};const o=W();e.innerHTML=`
  ...
  `;const n=document.getElementById("review-card"),u=document.getElementById("remembered-btn"),l=document.getElementById("forgotten-btn"),f=document.getElementById("skip-btn"),m=document.getElementById("shuffle-btn"),x=document.getElementById("session-correct"),c=document.getElementById("session-incorrect"),y=document.getElementById("words-pending");function b(){i=Y(),L(i),h()}function L(r){for(let $=r.length-1;$>0;$--){const T=Math.floor(Math.random()*($+1));[r[$],r[T]]=[r[T],r[$]]}return r}function d(){x.textContent=s.correct,c.textContent=s.incorrect}function h(){y.textContent=i.length}function p(){return i.length===0&&b(),i.shift()||null}function v(r){if(!r){n.innerHTML=`
      ...
      `,u.disabled=!0,l.disabled=!0,document.getElementById("restart-review")?.addEventListener("click",()=>{i=g(),L(i),s={correct:0,incorrect:0},d(),v(p())});return}t=r,a=!1,u.disabled=!0,l.disabled=!0;const $=r.reviewCount||0,T=k(r.difficulty||0);n.innerHTML=`
      <div class="review-card-inner">
        <div class="review-meta">
          <span class="tag type-tag">${ce(r.type)}</span>
          ...
          ${r.difficulty!==0?`<span class="tag difficulty-tag ${T.class}">${T.label}</span>`:""}
        </div>
    ...
    `;const j=document.getElementById("show-answer"),P=document.getElementById("review-answer");j.addEventListener("click",J);function J(){a||(a=!0,P.style.display="block",j.style.display="none",u.disabled=!1,l.disabled=!1,P.classList.add("fade-in"))}}function k(r){return r<=-2?{label:'<i class="fa-solid fa-star"></i> Fácil',class:"easy"}:r>=2?{label:'<i class="fa-solid fa-fire"></i> Difícil',class:"hard"}:{label:"",class:""}}function I(r){t&&(Z(t.id,r),r?(s.correct++,n.classList.add("correct-flash")):(s.incorrect++,n.classList.add("incorrect-flash"),i.push({...t,remembered:!1})),d(),h(),setTimeout(()=>{n.classList.remove("correct-flash","incorrect-flash"),v(p())},300))}u.addEventListener("click",()=>I(!0)),l.addEventListener("click",()=>I(!1)),f.addEventListener("click",()=>{t&&i.push(t),v(p())}),m.addEventListener("click",()=>{t&&i.unshift(t),L(i),v(p())}),document.addEventListener("keydown",q);function q(r){if(document.getElementById("review-card"))switch(r.code){case"Space":r.preventDefault(),a?v(p()):document.getElementById("show-answer")?.click();break;case"ArrowRight":a&&!u.disabled&&I(!0);break;case"ArrowLeft":a&&!l.disabled&&I(!1);break}}b(),v(p());const _=()=>{document.removeEventListener("keydown",q)};window._reviewCleanup=_}function ce(e){switch(e){case"word":return'<i class="fa-solid fa-font"></i> Palabra';case"phrasal":return'<i class="fa-solid fa-link"></i> Phrasal Verb';case"expression":return'<i class="fa-solid fa-comment"></i> Expresión';default:return'<i class="fa-solid fa-file"></i> Otro'}}const w=document.getElementById("app");window.addEventListener("offline",()=>{E("Sin conexión","Estás trabajando en modo offline.","warning",5e3),document.body.classList.add("offline-mode")});window.addEventListener("online",()=>{E("Conexión restaurada","Tus cambios se guardarán correctamente.","success",3e3),document.body.classList.remove("offline-mode")});window.addEventListener("error",e=>{console.error("Global error:",e.error),E("Error inesperado","Ha ocurrido un error. Intenta recargar la página.","error",0)});window.addEventListener("unhandledrejection",e=>{console.error("Unhandled promise rejection:",e.reason)});const F=document.querySelectorAll(".nav-link"),B=document.getElementById("theme-toggle");function de(){const t=R().theme||"dark";M(t)}function M(e){document.documentElement.setAttribute("data-theme",e);const t=B.querySelector("i");e==="dark"?(t.className="fa-solid fa-sun",B.title="Cambiar a modo claro"):(t.className="fa-solid fa-moon",B.title="Cambiar a modo oscuro");const a=R();a.theme=e,te(a)}function pe(){const t=document.documentElement.getAttribute("data-theme")==="dark"?"light":"dark";document.body.classList.add("theme-transitioning"),M(t),setTimeout(()=>{document.body.classList.remove("theme-transitioning")},300)}B.addEventListener("click",pe);window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",e=>{R().theme||M(e.matches?"dark":"light")});function ue(e){F.forEach(t=>{t.dataset.view===e?t.classList.add("active"):t.classList.remove("active")})}function z(e){window._reviewCleanup&&(window._reviewCleanup(),window._reviewCleanup=null),ue(e),w.style.opacity="0",w.style.transform="translateY(10px)",setTimeout(()=>{switch(e){case"home":oe(w);break;case"add":re(w);break;case"review":le(w);break;default:w.innerHTML="<p>Vista no encontrada</p>"}requestAnimationFrame(()=>{w.style.opacity="1",w.style.transform="translateY(0)"})},150)}w.style.transition="opacity 0.15s ease, transform 0.15s ease";F.forEach(e=>{e.addEventListener("click",t=>{t.preventDefault();const a=e.dataset.view;z(a)})});de();z("home");"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("./sw.js").then(t=>{console.log("SW registered: ",t)}).catch(t=>{console.log("SW registration failed: ",t)})});
